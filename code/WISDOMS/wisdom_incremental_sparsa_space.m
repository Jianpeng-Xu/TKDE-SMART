function [W, AS, B, C] = wisdom_incremental_sparsa_space(XS, Y, W_old, AS_old, B_old, C_old, lambda1, eta1, beta1, R)
...
if isnan(sum(vect_result))
    AS = AS_old; B = B_old; C = C_old; W = W_old;
else
...
CKA = kr(C, AS');
g_B =  - lambda1 * ((X2 - B * CKA') * CKA) + eta1 * (B - B_old);
