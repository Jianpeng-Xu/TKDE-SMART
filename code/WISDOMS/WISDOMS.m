function WISDOMS(responseIndex, numStartStations, stationInitScheme, randomSeeds,  R, lambda, eta, beta)
...
rng(0);
A = rand(s, R); B = rand(t, R); C = rand(d, R); W = rand(R,d);
pIndex = 1;
...
while s <S || t < T
    ...
    if strcmp(tos, 'time')
        ...
        Y_hat(spaceIndex,t) = (sum(X_T .* (A * W),2));
        ...
        % postUpdate
        rng(0);BT_old = rand(R,1);
        [W, A, BT, C] = wisdom_incremental_sparsa_time_postUpdate...
            (X_T, Y_T, W, A, BT_old, C, lambda, eta, beta, R);
        B = [B; BT'];
    elseif strcmp(tos, 'space')
        ...
        rng(0); AS_old = rand(R,1);
        [W, AS, B, C] = wisdom_incremental_sparsa_space...
            (X_S, Y_S, W, AS_old, B, C, lambda, eta, beta, R);
        A = [A; AS'];
    end
...
save(['WISDOMS-' num2str(responseIndex) '-' num2str(numStartStations) '-' num2str(stationInitScheme) '-' num2str(R) '-' num2str(lambda) '-' num2str(eta) '-' num2str(beta) '-' num2str(randomSeeds) '.mat'], ...
    'MAE_valid', 'MAE_test', 'MAE_ALL', 'MAE_test_station', ...
    'MAE_test_mean', 'MAE_valid_mean', 'W', 'A','B','C', ...
    'Y_hat', 'deszYMonth', ...
    'lambda', 'eta', 'beta', 'R');
