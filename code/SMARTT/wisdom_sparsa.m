function [V, A, B, C] = wisdom_sparsa(XT, Y, lambda, beta, R)

% This version only consider temporal latent factors in the regression part. 
% Hence, only V is optimized in this function
...
V = reshape(vect_result(length(A(:)) + length(B(:))+ length(C(:)) + 1 : length(A(:)) + length(B(:)) + length(C(:)) + length(V(:))), size(V));
end
...
V = reshape(parameterVect(length(A(:)) + length(B(:))+ length(C(:)) + 1 : length(A(:)) + length(B(:)) + length(C(:)) + R * d), [R, d]);

% compute f
XTA = XT(:,1:n,:);

BV = repmat(reshape(B(1:n,:)*V, [1, n, d]), [S, 1, 1]); 
BVY = sum(XTA .*  BV, 3) - Y;
loss = 0.5 * norm(BVY, 'fro')^2;
...
g_A = - lambda * (X1 - A * CKB')*CKB ; 

% B
X2 = double(tenmat(XT, 2));

CKA = kr(C, A);
g_B1 = reshape(squeeze(sum( bsxfun(@times, XTA, reshape(BVY, [S, n, 1])),1)), [n, d]) * V';
...
% ==================================================================
XTADPBVY = bsxfun(@times, XTA, reshape(BVY, [S, n, 1]));

% V
g_V = B(1:n, :)' * reshape(squeeze(sum(XTADPBVY, 1)), [n, d]);
% ==================================================================
