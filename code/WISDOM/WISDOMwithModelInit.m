function WISDOMwithModelInit(responseIndex, numStartStations, stationInitScheme, randomSeeds, R, lambda, eta, beta, InitialModel)
rng(randomSeeds);
addpath('../utils/');
...
% V = rand(R,d);
% A = InitialModel.A(1:s, 1:R); B = InitialModel.B(1:t, 1:R); C = InitialModel.C; W = InitialModel.W; 
V = zeros(R,d);

pIndex = 1;
while s <S || t < T
    % see if v=0 helps: not helpful  
%     V = zeros(R,d);
    % --------------
    p = pAll(pIndex);
    pIndex = pIndex + 1;
    ...
    elseif strcmp(tos, 'space')
        ...
        AS_old = InitialModel.A(s, :)';
        [W, V, AS, B, C] = wisdom_incremental_sparsa_space...
            (X_S, Y_S, W, V, AS_old, B, C, lambda, eta, beta, R);
        A = [A; AS'];
    end
...
save(['WISDOMwithModelInit-' num2str(responseIndex) '-' num2str(numStartStations) '-' num2str(stationInitScheme) '-' num2str(R) '-' num2str(lambda) '-' num2str(eta) '-' num2str(beta) '-' num2str(randomSeeds) '.mat'], ...
