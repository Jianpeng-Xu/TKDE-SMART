function [V, AS, B, C] = wisdom_incremental_sparsa_space(XS, Y, V_old, B_old, C_old, lambda1, eta1, beta1, R)
...
if isnan(sum(vect_result))
    AS = AS_old; B = B_old; C = C_old; V = V_old;
else
    ...
    V = reshape(vect_result(length(AS_old(:)) + length(B_old(:))+ length(C_old(:)) + 1 : ...
        length(AS_old(:)) + length(B_old(:)) + length(C_old(:)) + length(V_old(:))), size(V_old));
end
end
...
V = reshape(parameterVect(length(AS(:)) + length(B(:))+ length(C(:)) + 1 : ...
