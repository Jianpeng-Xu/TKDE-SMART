function [ V, A, BT, C] = wisdom_incremental_sparsa_time_postUpdate(XT, Y, V_old, A_old, C_old, lambda2, eta2, beta2, R)
...
if isnan(sum(vect_result))
    A = A_old; BT = BT_old; C = C_old; V = V_old;
else
    ...
    V = reshape(vect_result(length(A_old(:)) + length(BT_old(:))+ length(C_old(:)) + 1 : ...
        length(A_old(:)) + length(BT_old(:)) + length(C_old(:)) + length(V_old(:))), size(V_old));
end
end
...
V = reshape(parameterVect(length(A(:)) + length(BT(:))+ length(C(:)) + 1 : ...
...
g_A =  - lambda2 * (X1 - A * CKB')*CKB + eta2 * (A - A_old); 
